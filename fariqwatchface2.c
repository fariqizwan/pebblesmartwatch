...
static TextLayer *s_anim_layer;
static GFont s_time_font;
static GFont s_name_font;
static GFont s_date_font;
static GFont s_anim_font;
//Background image pointer
static BitmapLayer *s_background_layer;
static GBitmap *s_background_bitmap;
static Layer *s_draw_layer;

static PropertyAnimation *animlayer_property_animation;
...
static void trigger_animation(){
  // Set start and end
  GRect from_frame = layer_get_frame(text_layer_get_layer(s_anim_layer));
  GRect to_frame = GRect(20, 5, 100, 50);

  // Create the animation
  animlayer_property_animation = property_animation_create_layer_frame(text_layer_get_layer(s_anim_layer), &from_frame, &to_frame);
  
  // Get the Animation
  Animation *anim = property_animation_get_animation(animlayer_property_animation);
  ...
  animation_set_duration(anim, duration_ms); // milliseconds
  animation_set_delay(anim, delay_ms); // milliseconds
...
static void main_window_load(Window *window){
  ...
  s_name_layer = text_layer_create(
            GRect(0,PBL_IF_ROUND_ELSE(36,30),bounds.size.w,50));
  s_date_layer = text_layer_create(
            GRect(0,PBL_IF_ROUND_ELSE(116,110),bounds.size.w,50));
  s_anim_layer = text_layer_create(
  ...
  //Set the interface for s_anim_layer
  text_layer_set_background_color(s_anim_layer,GColorClear);
  text_layer_set_text_color(s_anim_layer,GColorWhite);
  //Create GFont
  ...
  s_anim_font = fonts_load_custom_font(resource_get_handle(RESOURCE_ID_BEYOND_MOUNTAINS_20));
  ...
  text_layer_set_font(s_anim_layer,s_anim_font);
  text_layer_set_text_alignment(s_anim_layer,GTextAlignmentCenter);
  static char name[] = "Fariq";
  text_layer_set_text(s_name_layer,name);
  static char dot[] = "\\_('_')_/";
  text_layer_set_text(s_anim_layer,dot);
  ...
  layer_add_child(window_layer,text_layer_get_layer(s_anim_layer));
...
static void main_window_unload(Window *window){
  ...
  text_layer_destroy(s_anim_layer);
  fonts_unload_custom_font(s_time_font);
  fonts_unload_custom_font(s_date_font);
  fonts_unload_custom_font(s_name_font);
  fonts_unload_custom_font(s_anim_font);
  //Destroy GBitmap;
...
static void update_time(struct tm *tick_time) {
  ...
  strftime(s_buffer,sizeof(s_buffer),clock_is_24h_style() ? "%H:%M %p" : "%l:%M %p",tick_time);
